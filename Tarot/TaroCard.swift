...
func setCardDirection(targetCardBtn:UIButton, isReversed:Bool){
    targetCardBtn.imageView?.transform = CGAffineTransform.identity
    if isReversed {
        targetCardBtn.imageView?.transform = CGAffineTransform.identity.scaledBy(x: 1, y: -1)
    }
}

let cardArray = [fool,magician,highPriestess]

let fool:TaroCard = TaroCard(
    cardNum: 0,
    cardName: "愚者",
    upright: "從零開始；好賭運；不墨守成規；追求新奇的夢想；冒險；放浪形骸；藝術家的氣質；異於常人；直攻要害、盲點；愛情狩獵者；愛情歷經滄桑；不拘形式的自由戀愛",
    reversed: "不安定；孤注一擲會失敗；缺乏責任感；損失；腳跟站不穩；墮落；沒發展；沒計劃；走錯路；行為乖張；輕浮的戀情；感情忽冷忽熱；不安定的愛情之旅",
    cardDetail:"正立的牌，暗示一段活在當下或隨遇而安的時期。在愛情中，則意味著你可能遇到一位喜歡目前生活，而不想計劃將來的伴侶。也可能是在形容你的伴侶，他（她）是一個難以捉摸的天真的人，或者，是一位不願受到任何長期計劃與關係約束的人。倒立的牌，表示你想從承諾中脫身，其實還不是時機。惟有對自己的未來有所承諾，或是解決了過去的問題，才能重新過上充滿自發性的生活。你也許渴望自由，想要以往過去種種，但現在還不是適當的時機。有時它也代表，你沒有在關鍵時刻把握住機會，或太固執於以前的計劃，過分依賴別人的建議。愚者的倒立，代表著「時機是關鍵所在」。",
    cardImg: UIImage(named: "00"))
